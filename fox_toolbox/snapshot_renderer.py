...
# 液态玻璃效果
_GLASS_FILL = (255, 255, 255, 180)    # 70% 透明白色
_GLASS_BORDER = (229, 231, 235, 100) # 40% 透明边框
_GLASS_SHADOW = (0, 0, 0, 20)         # 柔和阴影
...
def _make_background(size) -> Image.Image:
    """纯白色背景。"""
    return Image.new("RGBA", size, _BG_WHITE)
...
def _draw_glass_card(img: Image.Image, xy, title: Optional[str] = None, accent: Optional[Tuple] = None):
    ...
    if accent:
        ...
        # 底部轻微彩色光晕
        for i in range(ch // 3):
            alpha = int(20 * (1 - i / (ch // 3)))
            glow_draw.rectangle([0, ch - i - 1, cw, ch - i], fill=accent + (alpha,))
        glow = glow.filter(ImageFilter.GaussianBlur(_PX(20)))
...
def _draw_stat_cards(img, y, stats: MessageStats, db_table_count: int):
    ...
    for idx in range(6):
        ...
        f_val = _get_font(_PX(40), bold=True)
        ...
        vx = cx + (card_w - vw) // 2
        _draw_text(draw, (vx, cy + _PX(12)), val_str, f_val, _TEXT_DARK, img)
        
        # 标签文字
        lw = _text_width(draw, label, f_lbl)
        _draw_text(draw, (cx + (card_w - lw) // 2, cy + _PX(64)), label, f_lbl, _TEXT_MEDIUM, img)
...
def render_snapshot(
    ...
    # 8. 底部水印
    draw = ImageDraw.Draw(img)
    watermark_text = "由狐狸插件 /huli_record snapshot 生成 · 简洁现代风格 v2.2"
