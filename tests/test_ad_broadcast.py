...
def _bypass_magicmock_star():
    """根 conftest 把 astrbot.api.star.Star 设为 MagicMock，本测试用纯 object 基类替代。"""
    import astrbot.api.star as api_star_mod
    ...
    try:
        ...
        api_star_mod.Star = real
        m.AdBroadcastStar.__bases__ = bases
...
def _make_feature(tmp_path, config=None):
    cfg = _FakeConfig({
        "dsgg_enabled": True,
        "dsgg_platforms": [],
    ...
    if config:
        cfg.update(config)
    feature = AdBroadcastStar.__new__(AdBroadcastStar)
    feature.config = cfg
    feature.context = _FakeContext()
    feature.data_dir = tmp_path
    feature.ads = []
    feature.scheduled_times = []
    feature.known_groups = {}
    feature.broadcast_task = None
    feature.last_broadcast_minute = None
    feature.save_groups_pending = False
    feature._load_ads()
    feature._load_schedule()
    feature._load_groups()
    return feature
...
async def test_init_creates_empty_state(tmp_path):
    ...
    assert feature.ads == []
    assert feature.scheduled_times == []
    assert feature.known_groups == {}


@pytest.mark.asyncio
async def test_record_group_and_persist(tmp_path):
    feature = _make_feature(tmp_path)
    feature._record_group("aiocqhttp", "10001", "aiocqhttp:GroupMessage:10001/1", "group")
    feature._record_group("aiocqhttp", "10001", "aiocqhttp:GroupMessage:10001/2", "group")
    feature._record_group("telegram", "20001", "telegram:GroupMessage:20001/3", "group")
    feature._record_group("aiocqhttp", "10001", "aiocqhttp:GroupMessage:10001/4", "private")
    await feature._flush_groups()
    assert len(feature.known_groups) == 2
    assert feature.known_groups["aiocqhttp:10001"]["umo"] == "aiocqhttp:GroupMessage:10001/2"
    assert "aiocqhttp:10001/4" not in feature.known_groups


@pytest.mark.asyncio
async def test_platform_filter_whitelist(tmp_path):
    feature = _make_feature(tmp_path, config={"dsgg_platforms": ["aiocqhttp"]})
    feature._record_group("aiocqhttp", "1", "a:GroupMessage:1/1", "group")
    feature._record_group("telegram", "2", "t:GroupMessage:2/1", "group")
    targets = feature._get_targets()
...
async def test_disable_gids_both_formats(tmp_path):
    ...
    feature._record_group("aiocqhttp", "1", "a:GroupMessage:1/1", "group")
    feature._record_group("aiocqhttp", "2", "a:GroupMessage:2/1", "group")
    feature._record_group("telegram", "2", "t:GroupMessage:2/1", "group")
    feature._record_group("telegram", "3", "t:GroupMessage:3/1", "group")
    targets = feature._get_targets()
...
async def test_add_remove_list_ad(tmp_path):
    feature = _make_feature(tmp_path)
    ad_id = feature._add_ad([{"type": "Plain", "text": "广告"}], "广告文本")
    assert ad_id == 1
    assert feature.list_ads().startswith("广告列表")
    assert feature.get_ad(1)["text"] == "广告文本"
    assert "已删除广告 ID: 1" == feature.remove_ad(1)
    assert feature.get_ad(1) is None
    assert "未找到广告 ID" in feature.remove_ad(99)


@pytest.mark.asyncio
async def test_ads_persist_across_reload(tmp_path):
    feature = _make_feature(tmp_path)
    feature._add_ad([{"type": "Plain", "text": "x"}], "x")
    feature2 = _make_feature(tmp_path)
    assert len(feature2.ads) == 1
    assert feature2.ads[0]["text"] == "x"


@pytest.mark.asyncio
async def test_schedule_parse_and_invalid(tmp_path):
    feature = _make_feature(tmp_path)
    msg = feature.schedule("09:00,14:30,09:00")
    assert "09:00, 14:30" in msg
    assert feature.scheduled_times == ["09:00", "14:30"]
    feature.broadcast_task.cancel()
    await asyncio.gather(feature.broadcast_task, return_exceptions=True)
    feature.broadcast_task = None
    bad = feature.schedule("25:00")
    assert "时间格式错误" in bad
    no_arg = feature.schedule("")
    assert "当前定时广告时间" in no_arg
    feature.schedule("09:00")
    await asyncio.sleep(0)
    assert "已停止定时广告发送" in feature.stop_schedule()
    await asyncio.sleep(0)
    assert feature.scheduled_times == []
...
async def test_broadcast_sends_to_all_targets(tmp_path):
    feature = _make_feature(tmp_path)
    feature._add_ad([{"type": "Plain", "text": "打折啦"}], "打折啦")
    feature._record_group("aiocqhttp", "1", "a:GroupMessage:1/1", "group")
    feature._record_group("telegram", "2", "t:GroupMessage:2/1", "group")
    await feature._broadcast()
...
async def test_event_group_helpers(tmp_path):
    ...
    assert feature._event_platform(event) == "telegram"
    assert feature._event_group_id(event) == "888"
    platform, gid = feature._event_group(event)
...
async def test_group_list_output(tmp_path):
    ...
    feature._record_group("aiocqhttp", "1", "a:GroupMessage:1/1", "group")
    feature._record_group("aiocqhttp", "2", "a:GroupMessage:2/1", "group")
    text = feature.group_list()
    assert "广告群列表" in text
    assert "关闭" in text
    assert "启用" in text
    assert "暂无已接入的群聊" in _make_feature(tmp_path).group_list()
...
async def test_build_chain_and_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ad_main, "MessageChain", _MC)

    feature = _make_feature(tmp_path)
    chain = feature._build_chain([
        {"type": "Plain", "text": "hi"},
    ...
    chain2 = feature._build_chain([{"type": "Image", "path": str(local)}])
    assert chain2.chain[0].file == str(local)
    chain3 = feature._build_chain([{"type": "UnknownFoo", "x": 1}])
...
async def test_send_to_uses_chain(tmp_path, monkeypatch):
    ...
    await feature._send_to("umo:GroupMessage:1/1", ad)
...
async def test_schedule_persist(tmp_path):
    feature = _make_feature(tmp_path)
    feature.schedule("08:00")
    assert feature.broadcast_task is not None
    feature.broadcast_task.cancel()
    await asyncio.gather(feature.broadcast_task, return_exceptions=True)
    feature.broadcast_task = None
    feature2 = _make_feature(tmp_path)
    assert feature2.scheduled_times == ["08:00"]
