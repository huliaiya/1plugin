...
[![Version](https://img.shields.io/badge/Version-3.0.3-orange?style=for-the-badge)](CHANGELOG.md)
...
- 📤 **数据导入导出** — 支持 JSON / CSV / TXT / SQLite DB / SQL / ZIP（含媒体文件打包）格式，可跨实例迁移
...
| `dsgg_platforms` | `[]` | 参与广播的平台白名单；留空表示所有平台参与，填写平台名（如 `aiocqhttp`、`telegram`）则仅这些平台参与 |
...
| SQLite DB | `.db` | SQLite 数据库备份，保留完整消息字段，适合迁移和离线查询 |
| SQL | `.sql` | SQLite SQL 文本备份，便于审阅、版本管理和跨实例迁移 |
...
- 支持 JSON、CSV、SQLite DB、SQL、ZIP 格式
- SQLite DB 导入会执行只读完整性检查，并按 `messages` 表结构流式读取
- SQL 导入仅接受插件导出的 SQLite 备份语句，并执行受控校验后导入消息
...
| `/添加广告` | 发送指令后 30 秒内发送要添加的广告内容（支持富媒体），发送「取消」中止 | 管理员 |
